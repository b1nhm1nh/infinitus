...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "perf", effect: .read,
                       summary: "Process cost: CPU seconds so far, RSS bytes, thread count — sample twice to get an idle % (perf gate).",
                       replyShape: "{cpuSeconds, rssBytes, threads, uptimeSeconds}"),
