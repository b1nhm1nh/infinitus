...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "aws-login", args: ["<profile>"], options: ["--pid <session pid>", "--local"],
                       effect: .human,
                       summary: "Run the AWS sign-in for a profile: `aws login --remote` (or the SSO device-code flow) and report the URL to open elsewhere; --local opens this Mac's browser instead. Watch `aws-logins` for the state; --remote flows then need `aws-login-code`.",
