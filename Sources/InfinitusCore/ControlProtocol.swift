...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "status", effect: .read,
                       ...
                       replyShape: "{version, sha, engines:{cswap:{enabled,registered}, cliproxy:{enabled,registered,keyPresent}}, badge, signInRunning, playground, autoOrder}"),
        ...
        ControlCommand(name: "auto-order", args: ["on|off"], effect: .write,
                       summary: "Keep accounts sorted by headroom after every refresh, starred first (the Accounts pane toggle). Off lets reorder set the order by hand.",
                       replyShape: "{autoOrder}"),
        ControlCommand(name: "reorder", args: ["<fleet>", "<n>..."], effect: .write, requires: "reorder",
                       summary: "Set the rotation order: every account number exactly once, top first. Refused while auto-order is on (star accounts with prefer instead).",
                       replyShape: "{fleet}"),
