...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "forecast", effect: .read,
                       summary: "Run-rate projection: per window of the active account the measured pct/hour and the epoch instant it hits 100% (null when the reset lands first), plus when the fleet's weekly headroom is gone at that pace. Estimates.",
                       replyShape: "{forecast: {computedAt, active: {number, email, windows:[{name, pct, ratePctPerHour, resetsAt, hitsAt}]} | null, allDeadAt, basis} | null}"),
