...
const makeThread = (
  ...
  providerSessionId:
    source === "codex" ? "codex-session" : source === "omp" ? "omp-session" : CLAUDE_SESSION_ID,
...
it.layer(NodeServices.layer)("AgentSessionImporter", (it) => {
  describe("importRecentAgentThreads", () => {
    ...
    it.effect("stores the cursor shape the omp adapter resumes from", () =>
      Effect.gen(function* () {
        const bindings: Array<ProviderSessionDirectory.ProviderRuntimeBinding> = [];
        const scanner = AgentSessionScanner.AgentSessionScanner.of({
          scan: Effect.die("unused"),
        ...
        expect(bindings).toMatchObject([
          { provider: "omp", resumeCursor: { schemaVersion: 1, sessionId: "omp-session" } },
        ]);
