...
public enum Stats {
    ...
    private static func keyCalendar(_ calendar: Calendar) -> Calendar {
    ...
    public static func dayKey(_ date: Date, calendar: Calendar = .current) -> String {
        let p = keyCalendar(calendar).dateComponents([.year, .month, .day], from: date)
    ...
    public static func date(fromDayKey key: String, calendar: Calendar = .current) -> Date? {
        ...
        components.day = day
        return keyCalendar(calendar).date(from: components)
    }
