...
final class EngineRegistry: ObservableObject {
    ...
    /// settings name it (2026-09-04), else swapd's when swapd is on.
    /// Never a peer's: another machine's accounts must not drive this
    /// one's title, notifications or switch alert.
    var primary: FleetState? {
        if let routed = routedEngineID,
           let fleet = localFleets.first(where: { $0.provider == .claude && $0.engineID == routed }) {
            return fleet
        }
        return localFleets.first { $0.provider == .claude }
    }
