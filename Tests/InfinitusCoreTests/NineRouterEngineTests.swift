...
final class NineRouterEngineTests: XCTestCase {
    ...
    func testSnapshotLogsInOnceAndMapsTheClaudeFleet() async throws {
        ...
        XCTAssertEqual(seen.filter { $0.path.hasPrefix("/api/usage/") }.count, 2, "one usage call per enabled Claude connection")
        XCTAssertFalse(seen.contains { $0.path.hasPrefix("/api/usage/g1") })
    }
    ...
    func testUsageParseShapes() {
        if case .ok(let u) = NineRouterUsage.parse(Data(#"{"plan":"Claude Code","quotas":{}}"#.utf8)) {
            XCTAssertNil(u)
        ...
        if case .ok(let u) = NineRouterUsage.parse(Data(Self.usage.utf8)) {
